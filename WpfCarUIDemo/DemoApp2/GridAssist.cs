...
namespace DemoApp2 {
    public static class GridAssist {
        ...
        // 逻辑“内容列/行”索引（跳过像素分隔列/行），仅当未显式设置 Grid.Column/Row 时生效
        ...
        private static void OnGridLoaded(object sender, RoutedEventArgs? e) {
            ...
            var contentCols = Enumerable.Range(0, cols)
                .Where(i => !IsFixed(grid.ColumnDefinitions[i].Width)).ToArray();
            var contentRows = Enumerable.Range(0, grid.RowDefinitions.Count)
                .Where(i => !IsFixed(grid.RowDefinitions[i].Height)).ToArray();

            // ===== 将 ContentColumn/ContentRow 映射为物理 Column/Row（若未显式设置） =====
            foreach (UIElement item in grid.Children) {
                if (item.ReadLocalValue(Grid.ColumnProperty) == DependencyProperty.UnsetValue) {
                    int logicalCol = GetContentColumn(item);
                    if (logicalCol >= 0 && logicalCol < contentCols.Length)
                        Grid.SetColumn(item, contentCols[logicalCol]);
                }
                if (item.ReadLocalValue(Grid.RowProperty) == DependencyProperty.UnsetValue) {
                    int logicalRow = GetContentRow(item);
                    if (logicalRow >= 0 && logicalRow < contentRows.Length)
                        Grid.SetRow(item, contentRows[logicalRow]);
                }
            }
            ...
            // 先占位：手动设置 Row/Column 的元素
            foreach (UIElement item in grid.Children) {
                if (item.Visibility == Visibility.Collapsed) continue;

                bool manual = HasLocal(item, Grid.RowProperty) || HasLocal(item, Grid.ColumnProperty);
                if (!manual) continue;
        ...
        private static (int rows, int cols) ParseCounts(string spec) {
            ...
            if (parts.Length < 2)
                throw new FormatException("AutoRowColumn 必须形如 'rows,cols'，可用 '_' 代表自动扩。");

            int rows = parts[0] == "_" ? 1 : Math.Max(1, int.Parse(parts[0], CultureInfo.InvariantCulture));
            int cols = parts[1] == "_" ? 1 : Math.Max(1, int.Parse(parts[1], CultureInfo.InvariantCulture));
            return (rows, cols);
        }

        // 解析以逗号分隔的 GridLength 列表；空字符串返回空数组
        private static GridLength[] ParseGridLengthsList(string specs) {
            if (string.IsNullOrWhiteSpace(specs)) return Array.Empty<GridLength>();
